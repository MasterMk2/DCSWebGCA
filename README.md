...
DCS World Dedicated Server 向けのブラウザベース GCA (Ground Controlled Approach / 精密進入管制) コンソールです。
導入済みの **Tacview** のリアルタイム ACMI テレメトリストリームを TCP で受信し、WebSocket 経由でブラウザ上の PAR (精密進入レーダー) 画面に配信します。
...
[DCS Dedicated Server] --(Tacview addon)--> [ACMI real-time stream :34250/TCP]
        |
        v
[dcs-web-gca (Node.js)]  -- ACMI 2.2 パース -> 機体状態管理 -> GCA誘導計算
        |
        +-- HTTP  : 静的ファイル + REST API (/api/config, /api/state)
        +-- WS    : /ws で 5Hz スナップショット配信
        |
        v
[ブラウザ] 方位スコープ + 仰角スコープ + 進入データテーブル
```
...
- `gca.runways`: GCA 対象の滑走路を緯度経度・標高・方位・グライドパス角で定義 (複数可)。
...
| `WS /ws` | `hello` / `tracks` / `runwayChanged` メッセージ配信。`{"type":"selectRunway","runway":"..."}` で基準滑走路を切替 |
